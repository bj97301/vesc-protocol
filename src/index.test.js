  ...
  it('should parse long VESC messages (COMM_SET_MCCONF)', done => {
    ...
    let payload2 = Buffer.from(
      new Uint8Array([
        ...
        32, 144, 46, 76, 171, 169, 80, 61, 76, 204, 205, 65, 32, 0, 0, 67, 72, 0, 0, 68, 187, 128,
        0, 0, 0, 63, 102, 102, 102, 62, 76, 204, 205, 0, 10, 0, 0, 0, 10, 0, 5, 255, 255, 255, 255,
        255, 255, 255, 255, 67, 250, 0, 0, 69, 28, 64, 0, 0,
      ]),
    ) // 128 byte

    let payload3 = Buffer.from(
      new Uint8Array([
    ...
    let vescMessageParser = new VescMessageParser()
    let vescMessageHandler = new VescMessageHandler(vescMessageParser)
    let count = 0
    vescMessageParser.subscribe(message => {
      count += 1
      expect(message).toBeDefined()
