          ...
          switch (this.packetState) {
            case PACKET_HEADER:
              if (this.packet.packetType === 2) {
                ...
                logger.debug(
                  `PACKET_HEADER received short package: ${this.packet.packetType}, packetSize: ${this.packet.packetSize}`,
                )
              } else {
                this.packet.packetSize = byte << 8
                this.packetState = PACKET_LENGTH_SECOND
                logger.debug(
                  `PACKET_HEADER received long package: ${this.packet.packetType}, packetSize: ${this.packet.packetSize}`,
                )
              }
              break
            ...
            case PACKET_CRC_SECOND:
              ...
              if (this.packet.crc !== crc16(this.packet.payload)) {
                logger.debug(
                  `CRC "${crc16(this.packet.payload)}" doesn't match received CRC "${
                    this.packet.crc
                  }"`,
                )
                this.resetState()
